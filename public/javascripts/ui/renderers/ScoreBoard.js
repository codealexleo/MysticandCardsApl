class ScoreBoard {
    ...
    draw() {
        ...
        image(GameInfo.images.gold, 84, 824, 50, 50);

        // Desenhe o texto na frente da imagem da moeda
        textAlign(RIGHT, CENTER);
        textSize(24);
        textStyle(BOLD);
       // fill(255);
        text("x" + this.board.player.gold, 123, 850);
        text("x" + this.u  , 223, 780);

       // text("x" + this.board.player.gold, 1300, 30);    
        ...
        if (this.board.player.hp < 51 && this.board.player.hp > 40) {
            let color = "#33cc33";
            ctx.fillStyle = color;
        } else if (this.board.player.hp < 40 && this.board.player.hp > 30) {
            let color = "#dcf52f";
            ctx.fillStyle = color;
        } else if (this.board.player.hp < 30 && this.board.player.hp > 20) {
            let color = "#ff9933";
            ctx.fillStyle = color;
        ...
        if (this.board.opponents[0].hp < 51 && this.board.opponents[0].hp > 40) {
            let color = "#33cc33";
            ctx.fillStyle = color;
        } else if (this.board.opponents[0].hp < 40 && this.board.opponents[0].hp > 30) {
            let color = "#dcf52f";
            ctx.fillStyle = color;
        } else if (this.board.opponents[0].hp < 30 && this.board.opponents[0].hp > 20) {
            let color = "#ff9933";
            ctx.fillStyle = color;   
